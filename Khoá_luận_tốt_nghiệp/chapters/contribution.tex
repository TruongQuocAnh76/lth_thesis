\chapter{Đóng góp}
    \section{Thuật toán Improved Hybrid}
        ...
        \subsection{Đề xuất cải tiến}
            ...
            Ma trận thông tin Fisher (\gls{fim}) là $$\mathbf{F} = \mathbb{E}_{(x,y)\sim p_\text{data}}\left[\nabla_\theta \log p(y|x,\theta)\,\nabla_\theta \log p(y|x,\theta)^\top\right]$$
            Trace của $\mathbf{F}$ đo tổng độ cong của log-likelihood theo mọi hướng tham số. Thay vì tính FIM đầy đủ (chi phí $O(|\theta|^2)$), ta ước lượng vết thực nghiệm bằng chuẩn bình phương gradient:

            \begin{equation}
                \hat{\mathcal{F}} = \frac{1}{|\theta|} \cdot \frac{1}{B} \sum_{b=1}^{B} \lVert \nabla_\theta \mathcal{L}(x_b, y_b; \theta) \rVert^2
                \label{eq:fisher_trace}
            \end{equation}
            trong đó $B$ là số mini-batch dùng để ước lượng. Thay vì chọn $B$ theo kinh nghiệm, ta xác định $B$ bằng tiêu chí ổn định của bộ ước lượng: tăng dần $B$ cho đến khi phương sai tương đối của $\hat{\mathcal{F}}$ và mức thay đổi kéo theo của $r_k$ đều nhỏ hơn ngưỡng chấp nhận được trên tập hiệu chỉnh (calibration set) cố định. Đây là phép tính chi phí thấp: chỉ cần một lượt lan truyền thuận-nghịch (forward-backward pass), không cần nghịch đảo ma trận Hessian.
            ...
            \end{equation}
            trong đó $\sigma(\cdot)$ là hàm sigmoid, $\mathcal{F}_\text{ref}$ là giá trị tham chiếu được hiệu chỉnh từ mô hình dày đặc trên một tập con hiệu chỉnh cố định của tập huấn luyện, và $\alpha$ điều khiển độ nhạy. Cụ thể, $\mathcal{F}_\text{ref}$ được lấy là trung vị của $\hat{\mathcal{F}}$ trên một số lần khởi tạo ngẫu nhiên của cùng kiến trúc và bộ dữ liệu, nhờ đó đây là điểm cong "trung tính" của chiến lược thích nghi: khi $\hat{\mathcal{F}} = \mathcal{F}_\text{ref}$ thì $\sigma(\cdot)=0.5$ và $r_k = 0.65$. Khi $\hat{\mathcal{F}} \gg \mathcal{F}_\text{ref}$ (bề mặt nhọn), $\sigma(\cdot) \to 1$, và $r_k \to 0.5$; khi $\hat{\mathcal{F}} \ll \mathcal{F}_\text{ref}$ (bề mặt phẳng), $\sigma(\cdot) \to 0$, và $r_k \to 0.8$.
            ...
            \begin{equation}
                ...
                \label{eq:kd_loss}
            \end{equation}
            trong đó hàm chưng cất (distillation) dựa trên độ đo KL (KL-divergence) giữa phân phối softmax nhiệt độ $T$ của hai mô hình:
            ...
            Sau khi giai đoạn cắt tỉa một lần kết thúc, mạng đã khôi phục hiệu năng gần với mạng giáo viên. Tiếp tục dùng KD trong giai đoạn hình học (geometric) sẽ kìm hãm các điều chỉnh trọng số nhỏ mà cắt tỉa hình học (geometric pruning) cần thực hiện --- giáo viên đã không còn là mục tiêu phù hợp vì cấu trúc mạng đã khác. Do đó, $\mathcal{L}_\text{KD}$ chỉ được dùng khi $n = 0$ (bước cắt tỉa một lần), và bị tắt hoàn toàn với $n \geq 1$.


            Với vấn đề thứ ba, ở cuối giai đoạn cắt tỉa một lần, hồi quy cosine (cosine annealing) đã đưa \gls{learning_rate} về gần bằng 0. Khi giai đoạn lặp (iterative) bắt đầu và thực hiện bước tỉa đầu tiên, thuật toán giảm độ dốc (gradient descent) chỉ hoạt động trong vùng rất hẹp quanh điểm hội tụ cũ. Điều này hạn chế khả năng các trọng số còn lại tái tổ chức để bù đắp cho phần vừa bị tỉa.
            ...
            \begin{equation}
                ...
                \label{eq:lr_warmup}
            \end{equation}
            Giai đoạn khởi động $E_w$ không được chọn thủ công mà được hiệu chỉnh bằng ablation trên tập xác thực: thử các giá trị nhỏ của $E_w$ và chọn giá trị nhỏ nhất sao cho độ chính xác xác thực không còn cải thiện đáng kể so với cấu hình tốt nhất. Cơ chế này không phải là quay lại hoàn toàn (full rewinding) mà là một phiên bản nhẹ: chỉ áp dụng ở đầu mỗi bước lặp (iterative), không làm lại từ đầu lịch của giai đoạn ban đầu.
...
\begin{algorithm}[H] \caption{Hybrid Pruning Cải tiến -- Giai đoạn 2: Lịch học} \label{alg:hybrid_improved_lr_schedule} \DontPrintSemicolon \SetAlgoLined \KwIn{Bước tỉa $k$, epoch hiện tại $e$, số epoch tối đa $E_{\max}$, số epoch khởi động $E_w$, tốc độ học tinh chỉnh $\eta_{\mathrm{ft}}$} \KwOut{Tốc độ học $\eta$} \eIf{$k = 1$}{ $\eta \leftarrow \eta_{\mathrm{ft}} \cdot \dfrac{1}{2} \left( 1 + \cos\left( \dfrac{\pi e}{E_{\max}} \right) \right)$\; }{ \eIf{$e < E_w$}{ $\eta \leftarrow \eta_{\mathrm{ft}} \cdot \dfrac{e+1}{E_w}$\; }{ $\eta \leftarrow \eta_{\mathrm{ft}} \cdot \dfrac{1}{2} \left( 1 + \cos\left( \dfrac{\pi(e-E_w)}{E_{\max}-E_w} \right) \right)$\; } } \Return{$\eta$}\; \end{algorithm}
...
\end{algorithm}
    ...
    \section{Thiết lập thử nghiệm}
        \subsection{Bộ dữ liệu}
            ...
            \hline
            CIFAR-10  & 10  & $32\times32$ & 50\,000 & 10\,000 & RGB & chuẩn so sánh chính  \\
        ...
        Hai kiến trúc mạng nơ-ron được sử dụng trong luận văn nhằm đánh giá
    ...
    từ mạng nhỏ gọn đến mạng sâu có dung lượng lớn.
        ...
        Mỗi cấu hình thử nghiệm được chạy với 3 hạt giống ngẫu nhiên khác nhau; với mỗi cấu hình, kết quả được báo cáo dưới dạng trung bình cộng của 3 lần chạy $\pm$ độ lệch chuẩn mẫu tính trên chính 3 giá trị đó.
        ...
        \label{subsec:compression_metrics}
            ...
            \textbf{\gls{sparsity_distribution}} là biểu đồ nhiệt thể hiện tỷ lệ phần trăm tham số khác 0 trong mỗi lớp hoặc giai đoạn (stage) của mạng sau khi \pruning. Biểu đồ này giúp trực quan hóa cách độ thưa được phân bổ qua các tầng của mô hình, từ đó đánh giá xem \pruning có tập trung vào các lớp nào hay không.
        \begin{equation}
            s = 1 - \frac{\text{Số tham số khác 0}}{\text{Tổng số tham số}}
        \end{equation}
        Độ thưa (sparsity) đạt 90\% nghĩa là chỉ còn 10\% tham số khác 0.
