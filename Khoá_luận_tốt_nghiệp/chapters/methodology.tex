...
\newtheorem{theorem}{Định lý}[chapter]
    \section{Thuật toán baseline}
        ...
        \emph{(i)} xác định sớm mạng con quan trọng bằng các phương án huấn luyện chi phí thấp
        (learning rate lớn, huấn luyện độ chính xác thấp, kết hợp dừng sớm), và
        \emph{(ii)} tiếp tục huấn luyện chỉ mạng con đó đến độ chính xác mục tiêu.
        ...
        trên các kiến trúc VGG16, PreResNet101, ResNet18/50 và các bộ dữ liệu CIFAR-10/100, ImageNet.
        ...
        \paragraph{Tiêu chí tỉa dựa trên bảo toàn gradient.}
        ...
        \end{align}
        trong đó $\mathbf{Hg} = \mathbf{H}\mathbf{g}$ là tích vector-Hessian
        (Hessian-gradient product), $\mathbf{H} = \nabla^2\mathcal{L}(\symbfit{\theta}_0)$
        ...
        \paragraph{Điểm quan trọng của từng trọng số.}
        ...
        \begin{definition}[Tỉ lệ nén {\cite{synflow}}]
        ...
        \begin{definition}[Mặt nạ tỉa thưa {\cite{synflow}}]
        ...
        \begin{definition}[Synaptic saliency {\cite{synflow}}]
        \label{def:saliency}
        ...
        \begin{equation}
            ...
            \label{eq:saliency}
        \end{equation}
        Đây là tích Hadamard giữa gradient của $\mathcal{R}$ và véc-tơ tham số,
        tổng quát hóa nhiều tiêu chí điểm quan trọng kinh điển (Skeletonization,
        SNIP, Taylor-FO, \dots).
        \end{definition}
        
        \subsubsection{Định lý bảo toàn và hệ quả}
        ...
        \begin{theorem}[Bảo toàn theo nơ-ron {\cite{synflow}}]
        ...
        \begin{theorem}[Bảo toàn theo mạng {\cite{synflow}}]
        ...
        \noindent
        \textbf{Hệ quả -- Nguyên nhân gây sụp đổ tầng \cite{synflow}.}\;
        ...
        \subsubsection{Điểm Synaptic Flow và hàm mục tiêu SynFlow}
        ...
        \subsubsection{Điều kiện đủ để đạt Nén Tới Hạn Cực Đại}
        ...
        \begin{algorithm}[H]
        \caption{Iterative Synaptic Flow Pruning -- SynFlow \cite{synflow}}
        \label{alg:synflow}
        \DontPrintSemicolon
        \SetAlgoLined
        ...
        \subsection{Hybrid}
            ...
            \paragraph{Tiêu chí quan trọng theo độ lớn (magnitude criterion).}
            ...
            \paragraph{Bước cắt tỉa một lần lớn (one-shot step).}
            ...
            Một cách tương đương, $\tau_{p_k}$ chính là phân vị thứ $p_k$ của phân phối $\{|\theta_i|\}_{i=1}^{W}$.
            
            \paragraph{Số trọng số còn lại.}
            ...
            \paragraph{Lịch trình cắt tỉa hình học lặp lại (iterative geometric schedule).}
            ...
            \paragraph{Tinh chỉnh dựa trên patience (patience-based fine-tuning).}
...
\end{algorithm}
    ...
    \section{Hybrid Improved}
        ...
        \subsection{Đề xuất cải tiến}
            ...
            \paragraph{Ánh xạ Fisher trace sang $r_k$.}
            ...
            Để giải quyết vấn đề thứ hai, thay vì để task loss tự tìm lại biểu diễn tốt sau khi mạng bị tỉa mạnh, ta dùng chính mô hình dày đặc trước khi tỉa làm \emph{teacher} để hướng dẫn mạng đã tỉa (student) khôi phục các biểu diễn quan trọng.
            

            \paragraph{\textbf{Hàm mất mát distillation.}}
            Trước bước one-shot, ta lưu ảnh chụp (snapshot) $\theta_0$ của mô hình dày đặc. Sau khi tỉa tới tỉ lệ $r_k \cdot p$, ta tinh chỉnh mô hình đã tỉa $\theta_{pk}$ bằng hàm mất mát kết hợp:
