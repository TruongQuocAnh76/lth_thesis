\chapter{Nền tảng \& Tổng quan}
    ...
    \section{Phân loại thuật toán}
        Dựa trên các nghiên cứu hiện nay, các thuật toán tìm kiếm vé trúng thưởng có thể được phân nhóm dựa trên thời điểm thực hiện \pruning và cơ chế xác định tầm quan trọng của các kết nối nơ-ron.
        ...
        \subsection{Nhóm cắt tỉa sau huấn luyện}
            ...
            \gls{imp}: Là phương pháp nền tảng dùng để xác định các mạng con có khả năng huấn luyện độc lập. Thuật toán thực hiện chu kỳ huấn luyện - cắt tỉa - đặt lại (reset) trọng số về giá trị khởi tạo ban đầu \cite{imp}. Mặc dù \gls{imp} đạt hiệu suất cao ở mức độ thưa cực đại, chi phí tính toán của nó rất lớn do phải huấn luyện lại mạng nhiều lần.
        ...
        \subsection{Nhóm cắt tỉa hỗn hợp}
            \gls{hybrid_pruning}: Một phương pháp cắt tỉa mới trong đó phần lớn các trọng số không quan trọng (khoảng 60-80\%) được loại bỏ trong \gls{one_shot_pruning}, sau đó thực hiện một giai đoạn tinh chỉnh dài hơn. Tiếp theo, các chu kỳ cắt tỉa lặp lại theo cấp số nhân (geometric-like) được áp dụng cho các trọng số còn lại để đạt được độ thưa mục tiêu với độ chính xác tối ưu \cite{hybrid}.
...
\gls{imp} & Sau huấn luyện & \gls{magnitude} & Tiêu chuẩn vàng, chi phí cao \\ \hline

\gls{eb} & Giai đoạn sớm & Hệ số tỷ lệ \gls{bn} (BN Scaling) & Tiết kiệm 5.8x -- 10.7x năng lượng \cite{earlybird} \\ \hline

\gls{grasp} & Tại khởi tạo & Tín hiệu Gradient (Hessian-gradient) & Bảo toàn dòng chảy gradient \\ \hline

\gls{sf} & Tại khởi tạo & Dòng chảy neuron (Synaptic Flow) & Không cần dữ liệu, tránh sụp đổ lớp \\ \hline

\gls{hybrid_pruning} & Hỗn hợp & Độ lớn lặp lại kết hợp tinh chỉnh & Kết hợp ưu điểm \gls{one_shot_pruning} và \gls{iterative_pruning} \\ \hline
\end{tabularx}
