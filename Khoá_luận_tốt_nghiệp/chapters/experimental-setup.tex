...
từ mạng nhỏ gọn đến mạng sâu có dung lượng lớn.
    ...
    %     \hline
    %     \multirow{7}{*}{IMP}
    %     & \texttt{target\_sparsity}     & 0.3, 0.6, 0.9          & Tỷ lệ tham số bị cắt tỉa \\
    %     & \texttt{num\_iterations}      & 5                 & Số vòng lặp pruning \\
    %     & \texttt{epochs\_per\_iteration} & 160              & Số epoch mỗi vòng lặp \\
    ...
    % ----------------------------------------------------------
    \subsection{Chỉ số hiệu năng dự đoán}
    \label{subsec:accuracy}
    	...
    	\textbf{Accuracy Drop} ($\Delta$Acc, \%) đo mức suy giảm độ chính xác so với mô hình gốc (dense) trên cùng kiến trúc và bộ dữ liệu:
    \begin{equation}
    ...
    % ----------------------------------------------------------
    \subsection{Chỉ số nén và sparsity}
    \label{subsec:compression_metrics}

    	\textbf{Target Sparsity} (Sparsity mục tiêu) là tỷ lệ sparsity được đặt trước cho mỗi thử nghiệm (ví dụ: 30\%, 60\%, 90\%).

    	\textbf{Achieved Sparsity} (Sparsity đạt được) là tỷ lệ sparsity thực tế sau khi pruning, được tính theo công thức:

        \textbf{Sparsity Distribution} (Phân bố sparsity) là biểu đồ nhiệt thể hiện tỷ lệ phần trăm tham số khác 0 trong mỗi lớp hoặc stage của mạng sau khi pruning. Biểu đồ này giúp trực quan hóa cách sparsity được phân bổ qua các tầng của mô hình, từ đó đánh giá xem pruning có tập trung vào các lớp nào hay không.
    \begin{equation}
        s = 1 - \frac{\text{Số tham số khác 0}}{\text{Tổng số tham số}}
    \end{equation}
    Sparsity đạt 90\% nghĩa là chỉ còn 10\% tham số khác 0.

    	\textbf{FLOPs Saved} (\%, số phép tính giảm) là phần trăm số phép toán (FLOPs) được tiết kiệm so với mô hình dense, tính theo:
    \begin{equation}
