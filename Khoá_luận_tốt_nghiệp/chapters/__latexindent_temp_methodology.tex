...
Giả thuyết vé số may mắn, được đề ra bởi Frankle và Carbin (2018), cho rằng: "Với một mạng nơ-ron dày đặc feed-forward, được khởi tạo ngâu nhiên chứa một mạng con thưa mà khi huấn luyện một cách độc lập khỏi khởi tạo gốc, có độ chính xác tập test cao hơn hoặc bằng mạng gốc với cùng số lần lặp" \cite{article}. Vấn đề lớn nhất mà các bài báo về \gls{lth} nghiên cứu là về thuật toán để tìm ra tờ vé số may mắn này. Bài báo đầu tiên đề xuất ra LTH sử dụng thuật toán \gls{imp}. Phương pháp này dù chính xác nhưng đòi hỏi nhiều tài nguyên tính toán và thời gian. Bài luận này sẽ nghiên cứu và so sánh các phương pháp tìm kiếm vé số, đồng thời cải tiến một trong những phương pháp.
\section{Thuật toán cơ sở}
\subsection{Iterative Magnitude Pruning}
\gls{imp} là thuật toán nền tảng được Frankle và Carbin (2019) đề xuất để chứng minh \gls{lth}. \gls{imp} hoạt động dựa trên giả định rằng các trọng số có độ lớn (magnitude) nhỏ nhất là ít quan trọng nhất với hiệu suất mô hình \cite[p~.1]{fisher}. Khác với phương pháp cắt tỉa một lần, \gls{imp} thực hiện việc loại bỏ trọng số qua nhiều chu kỳ huấn luyện và đặt lại (resetting) giá trị, giúp "khử nhiễu" quá trình xác định tầm quan trọng của các kết nối nơ-ron.

Xét một mạng nơ-ron $f(x;\theta)$ với các tham số khởi tạo ban đầu $\theta_0 \sim D_\theta$ biểu thị rằng các trọng số ban đầu được khởi tạo theo phương pháp Kaiming. Một mạng con được xác định bởi một mặt nạ nhị phân (\gls{binary_mask}) $m \in \{0, 1\}^{|\theta|}$, có cùng kích thước với bộ tham số $\theta$.
...
Ngoài ra, thuật toán này còn áp dụng cơ chế đặt lại và đặt lại muộn (late resetting). Sau mỗi vòng cắt tỉa, các trọng số còn sót lại phải được đặt lại về giá trị khởi tạo ban đầu $\theta_0$ thay vì tiếp tục huấn luyện từ giá trị đã tối ưu. Bước này đã được chứng minh bằng thực nghiệm trong bài báo \gls{lth} gốc \cite[p~.2]{imp}, và được chứng minh một cách rõ ràng hơn trong \cite{deconstructlth}. \\
...
Huấn luyện mạng bằng Stochastic Gradient Descent (\gls{sgd}) trên tập huấn luyện $\mathcal{D}$
...
\paragraph{\textbf{Khoảng cách mask} (mask distance).}
Để đo lường mức độ ổn định của cấu trúc kết nối giữa các epoch liên tiếp, ta định nghĩa
\emph{khoảng cách mask} (\gls{mask_distance}) giữa hai epoch $t$ và $t-1$ bằng khoảng cách Hamming
...
toán tỉa thưa tại thời điểm khởi tạo (\gls{pruning_at_initialization}) được
...
Tỉ lệ thưa thớt (sparsity) tại thời điểm bất kỳ được định nghĩa là:
